...
class CarbonBuildMacOS(buildName: String, configType: String, preset: String) : BuildType({
    ...
    vcs {
        root(AbsoluteId("Carbon_Pathfinder_2_PathfinderFeatureKotlin"),"+:. => %github_checkout_folder%")
    ...
    triggers {
        vcs {
            triggerRules = "+:root=${AbsoluteId("Carbon_Pathfinder_2_PathfinderFeatureKotlin").id}:."
    ...
    features {
        pullRequests {
            vcsRootExtId = "${AbsoluteId("Carbon_Pathfinder_2_PathfinderFeatureKotlin")}"
            provider = github {
                authType = token {
